...
fmc_state_enum ob_erase(void)
{
	...
	if(FMC_READY == fmc_state)
	{
		...
		if(FMC_READY == fmc_state)
		{
			FMC_CTL &= ~FMC_CTL_OBER;// reset the OBER bit
			FMC_CTL |= FMC_CTL_OBPG;	// set the OBPG bit 
			...
			if(FMC_TOERR != fmc_state)// reset the OBPG bit 
			{
...
fmc_state_enum ob_write_protection_enable(uint16_t ob_wp)
{
	...
	if(0xFFU == (uint8_t)OB_WP0)
	{
		if (0xFFU == (uint8_t)OB_WP1)
		{
			if(FMC_READY == fmc_state)
			{
				...
				{
					OB_WP0 = ob_wrp0 ;
					fmc_state = fmc_ready_wait(FMC_TIMEOUT_COUNT);// wait for the FMC ready 
				}
	...
	else
	{
		if(FMC_READY == fmc_state)
		{
			...
			{
				FMC_CTL &= ~FMC_CTL_OBER;// reset the OBER bit 
				FMC_CTL |= FMC_CTL_OBPG;	// enable the option bytes programming
